...
const initialReports: AdminReport[] = [
  {
    ...
    proofType: 'image',
    proofCount: 2,
    submittedBy: 'PanamaRivals player',
    status: 'pending',
  },
  {
    id: 'report-002',
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="grid gap-3 sm:grid-cols-3">
        <SummaryCard label="Pending" value={pendingCount} accent="text-accent" />
        <SummaryCard label="Approved" value={approvedCount} accent="text-primary" />
        <SummaryCard label="Rejected" value={rejectedCount} accent="text-destructive" />
      </div>
      ...
      <div className="flex flex-col gap-4">
        {filteredReports.length ? (
          ...
          <div className="rounded-2xl border border-border bg-card p-10 text-center">
            ...
            <p className="font-medium">No reports match this filter.</p>
            <p className="mt-1 text-sm text-muted-foreground">
              Approved or rejected reports remain available under the corresponding filters.
            </p>
          </div>
        )}
      </div>
    ...
    <article className="rounded-2xl border border-border bg-card p-5 sm:p-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Report ID: <span className="font-mono">{report.id}</span> · Submitted by{' '}
            {report.submittedBy}
          </p>
        </div>
        {pending && (
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-[1fr_auto]">
        <div className="grid gap-3 sm:grid-cols-2">
          <Info label="Score" value={`${report.homeTeam} ${report.homeScore} – ${report.awayScore} ${report.awayTeam}`} />
          <Info label="Series" value={`Best of ${report.bestOf}`} />
          <Info label="Match date" value={report.matchDate} />
          <Info
            label="Proof"
            value={`${report.proofCount} ${report.proofType === 'image' ? 'screenshot' : 'replay'}${report.proofCount === 1 ? '' : 's'}`}
          />
        </div>
        <div className="flex min-w-36 items-center justify-center rounded-xl border border-dashed border-border bg-background p-4">
          ...
          <span className="text-sm text-muted-foreground">Review proof</span>
        </div>
      </div>
    </article>
  )
